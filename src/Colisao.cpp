...
int verificarColisao(double X1, double Y1, double Larg1, double Alt1, double X2, double Y2, double Larg2, double Alt2)
{
    if (X1 + Larg1 <= X2)
    {
        return 0;
...
void AplicarColisao()
{
    ...
    for (int i = 0; i < QuantidadeDinossauros; i++) {
        if (Dinossauros[i].Estado != 3) {
            ...
            if (verificarColisao(DinoX, DinoY, DinoLarg, DinoAlt, XObstaculo, YObstaculo, LarguraObstaculo, AlturaObstaculo) == 1) {
